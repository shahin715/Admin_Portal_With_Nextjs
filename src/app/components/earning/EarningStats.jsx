...
export default function EarningAndClosedOrders({
  ...
  closedOrderCardHeight = "h-full",
  earningValue = "16.4k",
  closedOrderPercent = 45,
}) {
  const isCompact = earningCardHeight === "h-[150px]" || earningCardHeight === "h-[100px]";
  ...
  return (
    <div className={`flex flex-col gap-4 ${cardWidth}`}>
      {/* Earning Card */}
      <Card
        className={`bg-zinc-900 border-zinc-700 text-white shadow-md ${earningCardHeight} ${
          isCompact ? "overflow-y-auto" : "overflow-hidden"
        }`}
      >
        <CardHeader className={`px-4 ${isCompact ? "pt-2 pb-1" : "pt-4 pb-2"} overflow-hidden`}>
        ...
        <CardContent className={`p-4 pt-0 ${isCompact ? "pb-1" : "pb-4"}`}>
          <div className={`flex items-end justify-between ${isCompact ? "h-12" : "h-20"}`}>
            {bars.map((height, i) => (
      ...
      {/* Closed Orders Card */}
      <Card
        className={`bg-zinc-900 border-zinc-700 text-white shadow-md ${closedOrderCardHeight} ${
          isCompact ? "overflow-y-auto" : "overflow-hidden"
        }`}
      >
